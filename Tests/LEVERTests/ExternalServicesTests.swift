...
final class GeminiMergeTests: XCTestCase {
    ...
    func testNullsAndBadDatesAreIgnored() throws {
        ...
        var doc2 = PurchaseDocument(rawText: "x", currencyCode: "INR")
        GeminiIntelligenceProvider.apply(try extraction(#"{"purchase_date":"9 September 2026"}"#), to: &doc2, confidence: 0.75)
        XCTAssertEqual(Calendar.current.component(.day, from: doc2.purchaseDate!), 9, "Human-readable dates from the model are still parsed strictly")
